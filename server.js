  ...
  db.get("SELECT price, promo_price FROM products WHERE id = 1", [], (err, product) => {
    ...
    db.all("SELECT key, value FROM settings", [], (err, settingsRows) => {
      ...
      const processOrder = (finalPrice) => {
        const payment_status = 'En attente de paiement (En ligne)';
        const status = 'Reçue';
        ...
        function(err) {
          ...
          // Perform a real production HTTPS API call to Maketou Pay to generate the secure online payment url
